...
file = horzcat(num2str(part_num),'.txt');
fileID = fopen(file,'r');
...
for i = 1:length(catchTrials)
    catchTrials(5,i) = num2cell(randi([1,12]));
    catchTrials(6,i) = num2cell(randi([1,12]));
    if eq(catchTrials{5,i},catchTrials{6,i})
        if catchTrials{5,i} == 12
...
for i = 1:21
    randTrials(i,1:12) = Shuffle(1:12);
    randTrials(i,13:24) = Shuffle(1:12);
    randTrials(i,25) = Randi(12);
end
counter = ones(1,21);
for i = 1:length(trials)
