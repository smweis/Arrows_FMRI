...
for which_run = run_number:numRuns
    ...
    if exist ((horzcat(num2str(part_num),'_',num2str(which_run),'.mat')),'file') == 2
        currTime = datetime('now','Format','yyyy-MM-dd''T''HHmmss');
        save(horzcat(num2str(part_num),'_',num2str(which_run),'_',char(currTime)));
    else
        save(horzcat(num2str(part_num),'_',num2str(which_run)));
    end
...
if exist ((horzcat(num2str(part_num),'_total.mat')),'file') == 2
    currTime = datetime('now','Format','yyyy-MM-dd''T''HHmmss');
    save(horzcat(num2str(part_num),'_total_',char(currTime)));
else
    save(horzcat(num2str(part_num),'_total.mat'));
end
